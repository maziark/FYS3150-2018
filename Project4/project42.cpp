...
int main(int argc, char* argv[]){
    ...
    if (my_rank == 0){
        ofile.open("outputNew.txt"); //File for energy and magnetization
        outfile.open("configsNew.txt"); //File for number of accepted configs
    }
    ...
    for (double temp = initial_temp; temp <= final_temp; temp += temp_step){}
        ...
        initialize(n_spins, spin_matrix, E, M, 0); //Kaller initialize

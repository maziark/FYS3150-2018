...
def MC_seasonal ():
    a_A = create_lambda(1, 2, 4.0)
    a_B = create_lambda(1, 0.5, 4.0)
    a_C = create_lambda(2, 2, 4.0)
    a_D = create_lambda(2, 0.5, 4.0)
    ...
    sirs_A = SIRS(400.0, a_A, b, c, d, d_i, e)
    sirs_B = SIRS(400.0, a_B, b, c, d, d_i, e)
    sirs_C = SIRS(400.0, a_C, b, c, d, d_i, e)
    sirs_D = SIRS(400.0, a_D, b, c, d, d_i, e)


    (t_array_A, S_avg_A, S_array_A, I_avg_A, I_array_A, R_avg_A, R_array_A) = sirs_A.MC_vital(300.0, 100.0, 50.0)
    (t_array_B, S_avg_B, S_array_B, I_avg_B, I_array_B, R_avg_B, R_array_B) = sirs_B.MC_vital(300.0, 100.0, 50.0)
    (t_array_C, S_avg_C, S_array_C, I_avg_C, I_array_C, R_avg_C, R_array_C) = sirs_C.MC_vital(300.0, 100.0, 50.0)
    (t_array_D, S_avg_D, S_array_D, I_avg_D, I_array_D, R_avg_D, R_array_D) = sirs_D.MC_vital(300.0, 100.0, 50.0)
    ...
    plt.title("a(t) = 1.0 * sin(2.0 * t) + 4.0")
    plt.xlabel ('Time')
    ...
    plt.title("a(t) = 1.0 * sin(0.5 * t) + 4.0")
    ...
    #plt.title("a = " + str(a(0)) + ", b = " + str(b(0)) + ", c = " + str(c(0)) + ", d = " + str(d(0))
    #    + ", d_i = " + str(d_i(0)) + ", e = " + str(e(0)))
    plt.title("a(t) = 2.0 * sin(2.0 * t) + 4.0")
    plt.xlabel ('Time')
    plt.ylabel ('Population')
    N = S_avg_A[-1] + I_avg_A[-1] + R_avg_A[-1]
    ...
    #plt.title("a = " + str(a(0)) + ", b = " + str(b(0)) + ", c = " + str(c(0)) + ", d = " + str(d(0))
    #    + ", d_i = " + str(d_i(0)) + ", e = " + str(e(0)))
    plt.title("a(t) = 2.0 * sin(0.5 * t) + 4.0")
    plt.xlabel ('Time')
    plt.ylabel ('Population')
    N = S_avg_A[-1] + I_avg_A[-1] + R_avg_A[-1]
    print ("population A : S/N = ", S_avg_A[-1]/N)
